...
static const uint8_t PROGMEM 
  Rcmd1[] = {                 // Init for 7735R, part 1 (red or green tab)
    ...
    ST7735_MADCTL , 1      ,  // 14: Memory access control (directions), 1 arg:
      MADCTL_MX | MADCTL_MY | MADCTL_RGB,                   //     row addr/col addr, bottom to top refresh
...
ST7735::ST7735(SPI * spi, uint8_t resetPin, uint8_t cdPin, uint8_t displayID):UI(ST7735_TFTWIDTH_128,ST7735_TFTHEIGHT_160){
	_spi = spi;
	_resetPin = resetPin;
	_cdPin = cdPin;
...
void ST7735::drawFastVLine(int16_t x, int16_t y, int16_t h, uint16_t color) {
	...
	if((x >= _width) || (y >= _height) || x < 0) return;
	if(y<0)y=0;
...
void ST7735::drawFastHLine(int16_t x, int16_t y, int16_t w, uint16_t color) {
	...
	if((x >= ST7735_TFTWIDTH_128) || (y >= ST7735_TFTHEIGHT_160) || y < 0){
		return;
	}
	if(x<0) x = 0;
	uint16_t temp; // avoid overflow
...
void ST7735::drawPixel(int16_t x, int16_t y, uint16_t color) {
	if((x < 0) ||(x >= ST7735_TFTWIDTH_128) || (y < 0) || (y >= ST7735_TFTHEIGHT_160)) return;
